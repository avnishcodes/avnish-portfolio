  ...
  const renderFormattedContent = (rawContent: string) => {
    ...
    return sections.map((sec, idx) => {
      ...
      if (trimmed.startsWith('```') && trimmed.endsWith('```')) {
        ...
        return (
          <div key={idx} className="my-6 rounded-2xl overflow-hidden border border-slate-800 bg-slate-950 shadow-xl">
            <div className="flex items-center justify-between px-4 py-2.5 bg-slate-900 border-b border-slate-800">
              <div className="flex items-center gap-2">
                <span className="w-3 h-3 rounded-full bg-rose-500/80 inline-block" />
                <span className="w-3 h-3 rounded-full bg-amber-500/80 inline-block" />
                <span className="w-3 h-3 rounded-full bg-emerald-500/80 inline-block" />
                <span className="ml-2 text-xs font-mono text-slate-400 font-semibold uppercase">{language}</span>
              </div>
              <button
                onClick={() => {
                ...
                className="text-xs font-mono text-slate-400 hover:text-white flex items-center gap-1 transition-colors px-2 py-1 rounded bg-slate-800/80 hover:bg-slate-700"
              >
